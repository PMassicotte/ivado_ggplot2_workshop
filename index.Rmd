...
It can be useful to reorder a categorical variable to make a plot easier to understand. One way to do it is with `forcats::fct_infreq()`. Note that the `forcats` library is not part of base R and must be installed separately.

.pull-left[

<small>
```{r reorder-factor, eval=FALSE}
ggplot(
  data = mpg,
  mapping = aes(x = forcats::fct_infreq(class)) #<<
) +
  geom_bar()
```
</small>
...
.pull-right[
```{r, ref.label="reorder-factor", echo=FALSE}

```
]

You can have a look to the `forcats::fct_reorder()` function to reorder a categorical variable using another variable.
